"""
Selective Rapidity-Sqrt Möbius Scan Transformer.

Data-dependent gating in log-complement space.  Each token learns its own
forget gate γ(x_t) and input gate α(x_t), making the recurrence selective
(Mamba-style) while keeping the state bounded in [0,1].

Recurrence:
    z_t = γ(x_t) · z_{t-1} + α(x_t) · log(1 − v_gated_t²)   # v_gated = gate·v
    s_t = sqrt(1 − exp(z_t))                    # ∈ [0,1] garantiert

where:
    γ(x_t) = sigmoid(W_γ x_t)     forget gate — how much past to discard
    α(x_t) = sigmoid(W_α x_t)     input gate — how strongly token enters
    v_t    = tanh(W_v x_t)        token velocity
    g_t    = sigmoid(W_gate x_t)  value gate

The recurrence z_t = γ_t·z_{t-1} + a_t (with a_t = α_t·log(1−v_t²))
is a first-order linear recurrence.  It is computed in O(log T) depth
via a parallel Blelloch scan over the associative operator:

    (a₁, γ₁) ∘ (a₂, γ₂) = (a₂ + γ₂·a₁, γ₁·γ₂)

This combines Mamba's selectivity with sqrt-coupling's geometric
boundedness — the first architecture to have both.
...
# ── Parallel scan for first-order linear recurrences ──
...
def sequential_linear_scan(a: torch.Tensor, gamma: torch.Tensor) -> torch.Tensor:
    """Sequential scan for z_t = γ_t · z_{t-1} + a_t.

    Simple O(T) loop.  At T ≤ 64 the overhead is negligible compared to
    the rest of the transformer.  Can be upgraded to parallel scan later.
    """
    B, T, H, D = a.shape
...
class SelectiveRapiditySqrtScanLayer(nn.Module):
    """Data-dependent gated scan in log-complement space.

    Each token produces four signals:
      v_t     = tanh(W_v x_t)       token velocity in (-1, 1)
      gate_t  = sigmoid(W_gate x_t) value gate
      γ_t     = sigmoid(W_γ x_t)    forget gate — how much past to discard
      α_t     = sigmoid(W_α x_t)    input gate — how strongly token enters

    The recurrence:
      a_t    = α_t · log(1 − gate_t · v_t²)
      z_t    = γ_t · z_{t-1} + a_t           (parallel scan)
      s_t    = sqrt(1 − exp(z_t))            bounded in [0, 1]
    """
    ...
    def __init__(self, d_model: int, d_head: int = 16, n_heads: int = 4,
        ...
        # Initialise forget/input gates to be "open" (small init → sigmoid≈0.5)
        # so the model starts near uniform cumsum and learns selectivity.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Additive term: a_t = α_t · log(1 − v_gated²)   # v_gated = v·gate (line above)
        w = v_gated * v_gated
...
class SelectiveRapiditySqrtTransformerLayer(nn.Module):
    def __init__(self, d_model: int, d_head: int = 16, n_heads: int = 4,
                 ffn_dim: int = None, dropout: float = 0.0, causal: bool = True):
        super().__init__()
        self.scan = SelectiveRapiditySqrtScanLayer(
...
class SelectiveRapiditySqrtTransformerLM(nn.Module):
...
def main():
    ...
    print("\n=== Selective Rapidity-Sqrt Möbius Scan Transformer (causal) ===")
    ...
    results.append((
        "Selective Rapidity-Sqrt Möbius Scan Transformer (MLM)",
        *run("Selective", model, X_train, Y_train, M_train, X_val, Y_val, M_val)
    ))

    print("\n=== RESULTS ===")
    for name, loss, ppl, acc, elapsed in results:
        print(
            f"{name:55s} | loss {loss:.4f} | ppl {ppl:7.2f} | "
