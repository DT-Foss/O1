...
> **What this section replaces.** The prior chat's "GSSM is a kernel" framing had one real result (§2: the RKHS readout) buried under three unmeasurable claims (a fixed-Mercer "unikernel," a capability narrative, a "Fable-5" packaging). This section keeps the real result at full strength, retracts the overclaim precisely, and adds the measured parallel-scan story that turns the algorithmic half of the claim from aspirational into verified.
...
Conflating these three objects is precisely the prior chat's error. Keeping them separate is the contribution.
...
### Adversarial caveats, folded in honestly (theory review)

The derivation survived adversarial scrutiny with **no structural fix required**. The flagged items are cosmetic and are corrected here in the framing so the paper does not inherit them:
...
## 5. What the prior chat got wrong vs right

| Prior-chat claim | Verdict | Why |
|---|---|---|
...
The split is clean: **§2 RKHS = real and corrected here; §3–5 unikernel/capability/Fable-5 = rejected as unmeasurable packaging.**
